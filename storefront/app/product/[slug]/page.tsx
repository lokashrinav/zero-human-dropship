...
import { CatalogPurchasePanel } from "@/app/product/[slug]/catalog-purchase-panel";
...
const ProductDetails = async ({ params }: { params: Promise<{ slug: string }> }) => {
	...
	return (
		<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
			...
			{!catalogPurchase && product.variants[0] && (
				<TrackProductView variant={product.variants[0]} name={product.name} />
			)}
			...
			<div className="lg:grid lg:grid-cols-2 lg:gap-16">
				...
				{allImages.length > 0 ? (
					...
					<div className="grid aspect-square place-items-center rounded-2xl border border-border bg-[radial-gradient(circle_at_center,rgba(190,242,100,0.16),transparent_46%)] text-center text-white/50 lg:sticky lg:top-24 lg:self-start">
						<div>
							<PackageOpen className="mx-auto size-20 text-lime-300/65" aria-hidden="true" />
				...
				<div className="mt-8 lg:mt-0 space-y-8">
					...
					{catalogPurchase && product.variants[0] ? (
						<CatalogPurchasePanel
							mode={catalogPurchase.catalogMode}
							paymentLink={catalogPurchase.paymentLink}
			...
			{!catalogPurchase && <ProductFeatures />}
